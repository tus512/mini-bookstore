...
import {
  ...
  Book as BookIcon, FileText, User, Sparkles, Compass, Baby, ChevronRight, Loader2
} from 'lucide-react';
...
export default function HomePage() {
  const {addToCart} = useBookstoreStore();
  ...
  // Fetch real books (first 12)
  const {data: booksPage, loading: loadingBooks} = useDoRequest<any>({
    url: '/books',
    isFetchOnLoad: true,
    params: {size: 12}
  });
  ...
  // Filter books for the slideshow (first 3 books from the real database)
  const heroBooks = books.slice(0, 3);

  // Featured bookshelf section (next 4 books)
      ...
      <div className="w-full font-sans bg-cream-bg select-none">
        ...
        <section className="relative overflow-hidden bg-cream-dark/40 py-16 md:py-24 border-b border-border-warm">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            {loadingBooks ? (
                <div className="flex flex-col items-center justify-center min-h-[400px] text-text-muted gap-2">
                  <Loader2 className="w-8 h-8 animate-spin text-primary" />
                  <span className="font-serif">Loading modern collections...</span>
                </div>
            ) : heroBooks.length === 0 ? (
                <div className="flex flex-col items-center justify-center min-h-[400px] text-text-muted">
                  <span className="font-serif text-lg">No books available in the catalogue.</span>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-12 min-h-[460px]">
                  ...
                  <div className="flex flex-col justify-center space-y-6 text-left transition-all duration-700 ease-in-out transform">
                    ...
                    <p className="text-sm text-text-muted max-w-lg leading-relaxed line-clamp-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4">
              {[
                ...
                {icon: <Headphones className="w-5 h-5"/>, title: '24/7 SUPPORT', desc: 'Dedicated helper desk'},
                  ...
                  <div key={idx} className="flex items-center gap-4 px-4 border-r-0 md:border-r border-border-warm/60 last:border-0 justify-center md:justify-start">
                    <div className="p-3 bg-primary-light rounded-full text-primary">{item.icon}</div>
                    <div className="text-left">
                      <h4 className="font-serif font-black text-sm text-text-dark">{item.title}</h4>
        ...
        <section className="py-20 select-none">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {loadingBooks ? (
                ...
                <p className="text-center text-text-muted font-serif py-12">No featured books found.</p>
            ) : (
            ...
            <div className="bg-cream-dark/40 rounded-2xl border border-border-warm overflow-hidden grid grid-cols-1 md:grid-cols-2 items-center">
              ...
              <div className="p-8 sm:p-12 lg:p-16 space-y-6 text-left">
                ...
                <p className="text-sm text-text-muted leading-relaxed max-w-md">
                  Discover your next favorite read from our handpicked collections. From heartwarming tales of personal
                  growth to edge-of-your-seat travels, we open the door to limitless worlds.
                </p>
                <div className="pt-2">
                  <Link
                      href="/explore"
