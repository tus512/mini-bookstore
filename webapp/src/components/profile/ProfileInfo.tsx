...
export default function ProfileInfo() {
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          ...
          {isEditing ? (
              ...
              <div
                  className="w-full bg-cream-dark/30 border border-border-warm/50 rounded-lg pl-10 pr-4 py-2.5 text-xs text-text-dark font-medium min-h-[38px] flex items-center">
  ...
  return (
      <div className="space-y-8">
        ...
        <div
            className="flex flex-col sm:flex-row items-center sm:items-start gap-6 p-6 bg-white border border-border-warm rounded-2xl shadow-sm">
          {/* Avatar */}
          <div className="relative shrink-0">
            <div
                className="w-24 h-24 rounded-full bg-primary flex items-center justify-center text-white font-serif font-black text-3xl shadow-md select-none">
              {initials}
            </div>
            {isEditing && (
                <button
          ...
          <div className="flex-1 text-center sm:text-left space-y-1">
            ...
            {user.role === 'ADMIN' && (
                <span
                    className="inline-flex items-center gap-1.5 text-[10px] font-bold text-primary bg-primary-light border border-border-warm px-2.5 py-1 rounded-full mt-1 uppercase tracking-wider">
        ...
        <div className="bg-white border border-border-warm rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-border-warm bg-cream-dark/20 flex items-center justify-between">
            <h3 className="font-serif font-black text-sm text-text-dark uppercase tracking-wider">Personal
              Information</h3>
            {isEditing && (
                <span
                    className="text-[10px] font-bold text-primary bg-primary-light px-2 py-0.5 rounded-full border border-border-warm animate-pulse">
              Editing…
            </span>
            )}
          </div>
