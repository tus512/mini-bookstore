...
export default function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full bg-cream-bg/95 border-b border-border-warm backdrop-blur-md transition-all duration-300 font-sans">
          ...
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center">
              <Link href="/" className="flex items-center gap-2 group">
                <div className="w-9 h-9 bg-primary rounded-lg flex items-center justify-center text-white shadow-sm group-hover:bg-primary-hover transition-colors">
                  <BookOpen className="w-5 h-5" />
                </div>
                <span className="font-serif font-extrabold text-2xl tracking-wider text-text-dark group-hover:text-primary transition-colors">
                  BOOKDOOR
                </span>
              </Link>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-1">
              {/* Search */}
              <Link href="/explore">
                <button className="p-2.5 text-text-muted hover:text-primary transition-colors rounded-full hover:bg-cream-dark cursor-pointer" title="Search Books">
                  <Search className="w-5 h-5" />
                </button>
              </Link>

              {/* Wishlist */}
              <Link href="/explore?filter=wishlist" className="relative p-2.5 text-text-muted hover:text-primary transition-colors rounded-full hover:bg-cream-dark" title="Wishlist">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden border-b border-border-warm bg-cream-bg px-4 pt-2 pb-6 space-y-1 animate-in fade-in slide-in-from-top-5 duration-200">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.path}
