...
import com.example.mini_bookstore.module.report.Report;
import com.example.mini_bookstore.module.report.ReportRepository;
...
import com.example.mini_bookstore.module.auth.PasswordUtil;
...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (categoryRepository.count() == 0 && bookRepository.count() == 0) {
            log.info("Starting database seeding for categories and books...");
            seedCategoriesAndBooks();
            seeded = true;
        }
        if (reportRepository.count() <= 5) {
            log.info("Starting database seeding for reports...");
            reportRepository.deleteAll();
            seedMockReports();
            seeded = true;
        }
        if (!userRepository.existsByEmail("admin@example.com")) {
            log.info("Starting database seeding for admin user...");
            seedAdminUser();
            seeded = true;
        }
        if (!seeded) {
    ...
    private void seedCategoriesAndBooks() {
        ...
        String[] subjects = { "Silent", "Echoes of", "Secrets of", "Beyond the", "Journey into", "The Lost",
                "Under the", "Shadows of", "The Golden", "Chronicles of", "Wonders of", "Art of", "Guide to",
                "Mastering", "History of", "Lessons from", "Path to", "Spirit of", "Call of", "In Search of" };
        String[] nouns = { "Waves", "Time", "Tomorrow", "Horizon", "Stars", "Forest", "Empire", "Light", "Darkness",
                "Wisdom", "Mindfulness", "Destiny", "Dreamer", "Ocean", "Mountains", "Silence", "Creativity",
                "Innovation", "Leadership", "Success" };
        String[] authors = { "J. R. Thorne", "Sarah Henley", "John Waves", "Dr. Alan Carter", "Elena Rostova",
                "Victoria Sterling", "Clara Greenwood", "M. J. Brown", "J. Watson", "D. Hayes", "Liam Vance",
                "Emma C. Clarke", "Lucas Sterling", "Aria Vance", "Robert Green", "Sophia Finch", "David Miller",
                "Isabella Cross", "Alexander Frost", "Grace Thorne" };
        ...
        for (int i = 1; i <= 300; i++) {
            ...
            String title = subjects[random.nextInt(subjects.length)] + " " + nouns[random.nextInt(nouns.length)]
                    + " Vol. " + (random.nextInt(5) + 1);
            ...
            String isbn = "978-" + (random.nextInt(900) + 100) + "-" + (random.nextInt(9000000) + 1000000) + "-"
                    + random.nextInt(10);

            String description = "An exceptional volume written by " + author + " detailing various concepts under "
                    + category.getName() +
            ...
            BigDecimal price = BigDecimal.valueOf(10.00 + (100.00 - 10.00) * random.nextDouble()).setScale(2,
    ...
    private void seedMockReports() {
        ...
        for (int i = 30; i >= 1; i--) {
            ...
            int orders = random.nextInt(12) + 3; // 3 to 14 orders
            int itemsSold = orders + random.nextInt(orders * 2 + 1); // 1x to 3x items
            BigDecimal revenue = BigDecimal.valueOf(itemsSold * (15.00 + random.nextDouble() * 45.00))
        ...
        reportRepository.saveAll(mockReports);
        log.info("Successfully seeded 30 mock sales reports!");
    }
    ...
    private void seedAdminUser() {
        User admin = User.builder()
                ...
                .fullName("Bookstore Administrator")
                .phone("1234567890")
                .role(Role.ADMIN)
                .build();
        userRepository.save(admin);
        log.info("Successfully seeded admin user admin@example.com!");
    }
}
